...
class MultinomialNBSummarizer(BasicSummarizer):
    def __init__(self, train_articles: list, train_labels: list, train_headlines: list, validate_articles: list, validate_labels: list, validate_headlines: list, language: str):
        ...
        tr_data = self.format_data(
            train_articles, train_labels)
        self.train_data = tr_data[0]
        self.train_labels = tr_data[1]

        self.train_features = self.vec.fit_transform(
            self.build_data(self.train_data, train_headlines)).toarray()
    ...
    def format_data(self, articles: list, label: list):
        ...
        return (list(zip(sentences, sentence_positions, sentence_topic_words, sentence_article_indexes)), sentence_labels)

    def extract_flat_sentence_attributes(self, articles: list):
    ...
    def merge_labels(self, labels: list):
    ...
    def build_data(self, data, headlines: list = None):
        return [self.extract_features(sent, pos, topic_words, headlines[article_idx] if headlines else None)
                for (sent, pos, topic_words, article_idx) in data]
    ...
    def summarize(self, sents: list, headline: str = None, num_of_sent: int = 5):
        ...
        for (idx, sent) in enumerate(sents):
            ...
            feature_array = self.vec.fit_transform(feature).toarray()
            predicted_label = self.classifier.predict(feature_array)
